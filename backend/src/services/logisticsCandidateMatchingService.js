...
function directRouteMatches(vehicle, shipment) {
  const cityMatch = normalizeLocation(vehicle.originCity) === normalizeLocation(shipment.pickupCity)
    && normalizeLocation(vehicle.destinationCity) === normalizeLocation(shipment.deliveryCity);
...
function directCandidate(vehicle, shipment) {
  ...
  return {
    ...
    baseDistanceKm: null,
    combinedDistanceKm: null,
    detourKm: 0,
    detourPercent: 0,
    estimatedCost: Number.isFinite(Number(vehicle.estimatedCost)) ? Number(vehicle.estimatedCost) : null
  };
}
